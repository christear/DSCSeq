        ...
        if(sum(colA) == 1){
            ...
            for(i in 1:ncol(psiA)){
                ...
                if(sum(colB) == 1){
                    ...
                    colnames(diff) = colnames(sums) = paste(condA,i,"-",condB,1:ncol(psiB),sep = "")
                }
                object@sc = cbind(object@sc,diff)
                object@me = cbind(object@me,sums)
            }
            if(sum(colB) == 1){
...
setMethod("rpresults","DSCSeqData",function(object,method = "mean"){
    if(method == "mean"){
        rpr = cbind(apply(object@me,1,mean),apply(object@sc,1,mean),apply(object@zscore,1,mean),object@qval,object@pval)
    }else if(method == "median"){
        rpr = cbind(apply(object@me,1,median),apply(object@sc,1,median),apply(object@zscore,1,median),object@qval,object@pval)
    }
...
### plot the results
setGeneric("scplot",function(object,mecut = NULL,qcut = NULL,pcut = NULL,sccut = NULL,method = NULL,main = NULL,xlab = NULL,ylab = NULL,xlim = NULL,ylim = NULL,pch = NULL,cex = NULL) standardGeneric("scplot"))

setMethod("scplot","DSCSeqData",function(object,mecut = log2(400),qcut = 0.01,pcut = 0.001,sccut = 0,method = "density",main = NULL,xlab = "log2 Mean expression",ylab = "Delta PSI/PIR",xlim = NULL,ylim = NULL,pch = 19,cex = .8){
    rpr = rpresults(object)
